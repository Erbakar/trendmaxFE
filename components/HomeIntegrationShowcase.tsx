...
import { ENTEGRASYON_LOGOS, PAZARYERI_DEMO_URL, type HomeIntegrationLogo } from '../data/homePage';
...
function MarqueeRow({
  ...
  return (
    <>
      <div className="mobile-horizontal-scroll flex snap-x snap-mandatory gap-3 overflow-x-auto px-8 py-2 md:hidden">
        {items.map((item) => (
          <div key={`mobile-${item.logoUrl}`} className="shrink-0 snap-center">
            <IntegrationLogoCard item={item} />
          </div>
        ))}
      </div>
      <div className={`hidden md:flex ${reverse ? 'animate-marquee-integrations-reverse' : 'animate-marquee-integrations'}`}>
        {loop.map((item, i) => (
...
const HomeIntegrationShowcase: React.FC = () => {
  ...
  return (
    <section className="relative overflow-hidden border-y border-orange-500/20 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-16 text-white lg:py-24">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 text-center">
        ...
        <div className="relative mb-12 -mx-4 sm:mx-0">
          ...
          <div className="py-2 md:overflow-hidden">
